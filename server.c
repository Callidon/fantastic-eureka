...
int main(int argc, char **argv) {
  ...
  if (setsockopt(socket_descriptor, SOL_SOCKET, SO_REUSEADDR, &(int){1},
                 sizeof(int)) < 0) {
    perror("Erreur : l'opération setsockopt(SO_REUSEADDR) a échouée");
    error_stop();
  }

  /* association du socket socket_descriptor à la structure d'adresse
   * adresse_locale */
  ...
  for (;;) {
    ...
    /* lancement du handler chargé de gérer les interactions avec le nouveau
     * client */
    if (pthread_create(&array_client->clients[client_ind]->client_thread, NULL,
                       server_handler, (void *)datas) < 0) {
      perror("Erreur : impossible de créer le thread pour gérer la connexion "
             "avec le client");
      error_stop();
    }
